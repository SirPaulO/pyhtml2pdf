...
```bash
HTML2PDF$ sam local invoke HelloWorldFunction --event events/sqs-event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
HTML2PDF$ sam local start-api
HTML2PDF$ curl http://localhost:3000/
```
...
```bash
HTML2PDF$ sam logs -n HelloWorldFunction --stack-name "html2pdf" --tail
```
...
```bash
HTML2PDF$ pip install pytest pytest-mock --user
HTML2PDF$ python -m pytest tests/ -v
```
